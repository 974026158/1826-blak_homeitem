/*点击查号隐藏固定栏和遮罩层*/
	...
	$(".rightInput").blur(function(){
		...
		if($(".rightInput").val() == "") {
			$s1.html("请输入手机号")
			$s1.css("color", "red").css("font-size",12)
		} else if(reg.test($(".rightInput").val())) {
			$s1.html("格式正确")
			$s1.css("color", "gold").css("font-size",12)
			$test.click(function(){
				$(".container").show()
				...
				$s2.html("请完成验证").css("color","gold");
			})
		} else {
			$s1.html("格式不正确")
			$s1.css("color", "red").css("font-size",12)
			$test.click(function(){
				$s2.html("请输入手机号").css("color","red");
			})
		}
	})
	$test.click(function(){
		$s2.html("请输入手机号");
		$s2.css("font-size",12).css("color","red")
	})
